...
for x in $(echo *.pl *.py *.rb 2>/dev/null); do
    ...
    if [ "$TRAVIS_PERL_VERSION" = "5.8" ]; then
        ...
        [ "$x" = "check_mongodb_master.pl" ] && { echo "skipping check_mongodb_master.pl on Perl 5.8 since the MongoDB module requires Type::Tiny::XS which breaks for some unknown reason"; continue; }
    fi
    set +e
