...
- ```check_elasticsearch_*.pl``` - checks Elasticsearch cluster state / shards / replicas, nodes & data nodes online, single node ok, specific node found in cluster, per node stats, per index existence / shards / replicas / settings / age / stats, elasticsearch / lucene versions
