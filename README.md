...
- `check_hadoop_*.pl/py` - various Apache Hadoop monitoring utilities for HDFS, YARN and MapReduce (both MRv1 & MRv2):
  ...
  - Yarn - app last finished state / user / queue / elapsed time (batch job SLAs), app running (check long living yarn service is still alive), long running jobs detection (detect SLAs breaches for in-progress batch jobs or forgotten Spark Shells holding resources), unhealthy NodeManagers, queue state, queue capacity, app stats, % of memory allocated
