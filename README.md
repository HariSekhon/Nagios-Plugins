...
- ```check_mapr_*.pl``` - MapR checks via MapR Control System API - checks services, MapR-FS space, node health, nodes available, MapR-FS disk utilization, failed disks, CLDB heartbeats, MapR alarms, MapReduce mode and memory utilization, disk and role balancer metrics
