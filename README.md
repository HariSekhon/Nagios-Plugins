...
- ```check_redis_*``` - check Redis API writes/reads/deletes with timings, check connected clients, gather statistics
- ```check_riak_*``` - check Riak API writes/reads/deletes with timings, checks diagnostics, checks nodes agree on ring status, gather statistics
