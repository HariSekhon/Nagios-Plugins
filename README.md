...
- ```check_hadoop_*.pl``` - various Hadoop monitoring utilities for HDFS & MapReduce including cluster balance, replication, space, block count limits, dead Datanodes and TaskTrackers, blacklisted TaskTrackers, Namenode, JobTracker and Yarn Resource Manager heap usage, gather metrics
...
- ```check_ambari.pl``` - Hortonworks Ambari Hadoop management server - uses Ambari API to report the status of services and nodes managed by Ambari
