...
- ```check_hbase_*.pl``` - various HBase monitoring utilities, checking Masters/Backup Masters, RegionServers, table availability, unassigned regions, gather metrics
- ```check_cloudera_manager_*.pl``` - check states and health of Cloudera Manager Hadoop services/roles/nodes/mgmt svcs, config staleness, Cloudera Enterprise license expiry, Cloudera Manager and CDH cluster versions, utility switches to list clusters/services/roles/nodes as well as list users and their role privileges, fetch a wealth of Hadoop & OS monitoring metrics from Cloudera Manager. Modern Hadoop users with Cloudera Manager will want to use these (Disclaimer: I worked for Cloudera, but seriously CM collects an impressive amount of metrics making check_cloudera_manager_metrics.pl alone a very versatile program from which to create hundreds of checks)
- ```check_ambari.pl``` - Hortonworks Ambari Hadoop management server - this plugin reports the status of services and nodes managed by Ambari
...
Check out the adjacent ToolBox repo https://github.com/harisekhon/toolbox for other useful tools such as Hadoop HDFS per block read performance + location debugging tool (hadoop_hdfs_time_block_reads.jy), watch_url.pl for load balanced environments and other useful programs.
