...
- `check_linux_*` - checks context switches, interface errors / promiscous mode disabled / duplex / speed / MTU / stats, load normalized per CPU core (more useful than the default check_load which would need difference configs for heterogenous hardware), ram used, system file descriptors used, timezone settings, users / groups present (eg. PAM/LDAP integration), duplicate UID/GUIDs, groups.allow contains only specific groups
...
- self-timeouts
- graphing data where appropriate (use [PNP4Nagios](https://docs.pnp4nagios.org/) for automatic graphing)
