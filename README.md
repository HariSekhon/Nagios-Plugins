...
- ```check_selenium_hub_*.py``` - checks [Selenium Grid](https://www.selenium.dev/documentation/en/grid/) Hub / [Selenoid](https://aerokube.com/selenoid/) status is ready, and that browsers are available by requesting a browser (eg. chrome or firefox), querying a url and optionally performing a content or regex match on the returned web page
