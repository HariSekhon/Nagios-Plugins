...
- `check_hadoop_*.pl/py` - various Apache Hadoop monitoring utilities for HDFS, YARN and MapReduce (both MRv1 & MRv2):
  ...
  - Hadoop general - Masters status and High Availability (ZKFC, active/standby status), nodes counts, dead nodes / blacklisted / unhealthy nodes, masters heap usage, general gather metrics and JMX information
...
- ```check_presto_*.py``` - Presto SQL DB - checks status, if coordinator, number of current queries, number of failed nodes, environment, version
