...
Most Python HTTP Rest-base plugins for systems with authentication have a `--kerberos` switch which supercedes `--username/--password` and will either use the TGT from the environment cache (`$KRB5CCNAME`) or if `$KRB5_CLIENT_KTNAME` is present will kinit from the keytab specified in `$KRB5_CLIENT_KTNAME` to a unique path per plugin to prevent credential clashes.
