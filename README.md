...
- ```check_ambari.pl``` - Hortonworks Ambari Hadoop management server - reports the status of services and nodes managed by Ambari
- ```check_ibm_biginsights_*.pl``` - IBM BigInsights Hadoop distribution checks including services, nodes, agents, BigSheets workbook runs, dfs paths and properties, HDFS space and block replication, BI console version, BI console applications deployed
