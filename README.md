...
- `check_hadoop_*.pl/py` - various [Apache Hadoop](http://hadoop.apache.org/) monitoring utilities for HDFS, YARN and MapReduce (both MRv1 & MRv2):
  ...
  - Yarn - app last finished state / user / queue / elapsed time (batch job SLAs), app running (check long living yarn service is still alive), long running jobs detection (detect SLAs breaches for in-progress batch jobs or forgotten Spark Shells holding resources), unhealthy NodeManagers, queue state, queue capacity, app stats, % of memory allocated, metrics with optional thresholds to check things such as activeNodes, appsPending, lostNodes, unhealthyNodes etc.
...
- ```check_apache_drill_*``` - [Apache Drill](https://drill.apache.org/) status and metrics for a given node, apply thresholds to a given metric or return multiple or all metrics
...
- ```check_ranger*``` - [Apache Ranger](https://ranger.apache.org/) checks:
...
- ```check_*_version*``` - checks running versions of software - primarily written to detect version inconsistencies across large clusters of servers and failed/partial upgrades across large automated infrastructures. Now also used to check [Docker](https://www.docker.com/) tagged images contain the right version of the expected software (which double validates that software programs in this and other repos are being tested against all the expected versions)
