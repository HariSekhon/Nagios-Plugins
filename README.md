...
- ```check_cloudera_manager_*.pl``` - check states and health of Cloudera Manager Hadoop services/roles/nodes/mgmt svcs, config staleness, Cloudera Enterprise license expiry, Cloudera Manager and CDH cluster versions, fetch a wealth of Hadoop & OS monitoring metrics from Cloudera Manager. Modern Hadoop users with Cloudera Manager will want to use this (Disclaimer: I worked for Cloudera, but seriously CM collects an impressive amount of metrics making this a very versatile program from which to create hundreds of checks from check_cloudera_manager_metrics.pl alone)
