...
- ```check_hbase_*.pl``` - various HBase monitoring utilities using Thrift + Stargate APIs, checking Masters /Backup Masters, RegionServers, table availability, unassigned regions, regions stuck in transition, region count balance across RegionServers, compaction in progress (by table and by regionserver), number of regions in transition, longest current region migration time, hbck status and inconsistencies, gather metrics
...
- ```check_travis_ci_last_build.py``` - checks the last build status of a given Travis CI repo, verbose mode gives the commit details too as well as builds in process info & stats
