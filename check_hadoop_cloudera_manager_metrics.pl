...
%options = (
    ...
    "A|activityId=s"   => [ \$activity,     "ActivityId to collect metric for (eg. MapReduce). Requires --cluster and --service" ],
    "N|nameservice=s"  => [ \$nameservice,  "Nameservice to collect metric for. Requires --cluster and --service" ],
    "R|roleID=s"       => [ \$role,         "RoleId to collect metric for (eg. hdfs4-NAMENODE-73d774cdeca832ac6a648fa305019cef - use --list-roleIds to find CM role ids for a given service). Requires --cluster and --service" ],
