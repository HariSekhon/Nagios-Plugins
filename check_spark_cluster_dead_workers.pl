...
if( ( $alive_workers + $dead_workers ) != $total_ever_workers ){
    quit "UNKNOWN", "ERROR alive ($alive_workers) + dead workers ($dead_workers) != total ever workers ($total_ever_workers)!!! $nagios_plugins_support_msg"
}
